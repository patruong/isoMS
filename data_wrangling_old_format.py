...
def get_filtered_data(ms_file, scan_type = "MS", amino_acids = ["P", "Hyp"]):
    ...
    df = df.loc[:, ["seqNum", "peak", "isoratio", "ion", "n"]] # columns to filter on
    df.isoratio = df.isoratio / df.n
...
def transform_df_boxplot(df, peak):
    """
    ...
    for category in df["ion"].unique():
        ...
        var_x = df_category[df_category["scan_type"] == "MS"].describe()
        var_y = df_category[df_category["scan_type"] == "MSMS"].describe()
...
df_boxplot = transform_df_boxplot(df, peak = "13C")
df_boxplot.to_csv("TEST_13C.csv", index = False, sep = ";")
df_boxplot = transform_df_boxplot(df, peak = "15N")
df_boxplot.to_csv("TEST_15N.csv", index = False, sep = ";")
df_boxplot = transform_df_boxplot(df, peak = "2H")
df_boxplot.to_csv("TEST_2H.csv", index = False, sep = ";")
df_boxplot = transform_df_boxplot(df, peak = "18O")
df_boxplot.to_csv("TEST_18O.csv", index = False, sep = ";")
...
#############################################
# Check relationship between MS and MSMS ####
#############################################

df_ion = df[df["scan_type"] == ]
